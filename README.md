...
## Запуск сервиса
```bash
task/make docker // Пересобирает и перезапускает Docker-контейнеры
```


## Taskfile/Makefile
- `docker`: Пересобирает и перезапускает Docker-контейнеры. Сначала выполняется задача clean-docker, а затем docker compose up.
- `clean-docker`: Останавливает и удаляет Docker-контейнеры и связанные с ними образы.
